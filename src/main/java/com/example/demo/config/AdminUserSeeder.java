...
public class AdminUserSeeder implements ApplicationRunner {
    ...
    public void run(ApplicationArguments args) {
        ...
        if (!StringUtils.hasText(email)) {
            throw new IllegalStateException("Admin seeding requires SEED_ADMIN_EMAIL");
        }
        ...
        if (userService.existsByEmail(email)) {
            log.info("Admin user '{}' already exists — skipping seeding", email);
            return;
        }
        ...
        log.info("Seeded admin user '{}' with role {}", email, Roles.PLATFORM_ADMIN);
    }
}
