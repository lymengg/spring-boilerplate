...
### Roles
Seven built-in roles: `ADMIN` (Administrator), `USER` (User), `USER_MANAGER` (User Manager), `MANAGER` (Manager), `EMPLOYEE` (Employee), `AUDITOR` (Auditor), `FINANCE` (Finance).
...
- **User management**: Privilege hierarchy prevents managing users with equal/higher permissions. Last admin protection prevents deleting/disabling the last Administrator in a tenant.
